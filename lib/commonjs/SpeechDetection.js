...
class SpeechDetection extends _eventemitter.EventEmitter {
  ...
  async init(config = {}) {
    ...
    if (!this.encoder) {
      this.encoder = new _lamejs.default.Mp3Encoder(1, this.config.sampleRate, 128);
    }
  }
  setupEventListeners() {
  ...
  async startRecording() {
    if (!this.recording) {
      try {
        ...
        await _RNRecordSpeech.default.start();
        this.recording = true;
  ...
  onSendSpeechData = async () => {
    ...
    if (this.chunks.length > 0) {
      ...
      this.chunks = [];
      this.tempChunks = [];
      const arrayBuffer = await (0, _bufferutils.concatenateBase64BlobsToArrayBuffer)(allChunks);
